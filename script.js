  ...
  products: [
    {
      ...
      categoryLabel: "Retatrutide",
      descriptor: "Hero metabolic research product",
      retail: "Rp 1.200.000",
      partner: "Rp 900.000",
      accent: "#0F7778",
      image: "retatrutide-pack.webp",
      featured: 1,
    },
    {
      key: "retatrutide30",
      ...
      category: "retatrutide",
      categoryLabel: "Retatrutide",
      descriptor: "Higher-strength metabolic research product",
      retail: "Rp 1.900.000",
      partner: "Rp 1.500.000",
      accent: "#0F7778",
      image: "products/Retatrutide30.png",
      featured: 2,
    },
    {
      key: "ghkcu",
      ...
      descriptor: "Copper peptide research product",
      retail: "Rp 650.000",
      partner: "Rp 500.000",
      accent: "#6667C8",
    ...
    },
    {
      ...
      category: "peptide",
      categoryLabel: "Research peptide",
      descriptor: "Peptide research product",
      retail: "Rp 1.200.000",
      partner: "Rp 900.000",
      accent: "#1769AF",
    ...
    },
    {
      ...
      descriptor: "Metabolic research product",
      retail: "Rp 800.000",
      partner: "Rp 600.000",
      accent: "#C89224",
    ...
    },
    {
      ...
      category: "peptide",
      categoryLabel: "Research peptide",
      descriptor: "Peptide research blend",
      retail: "Rp 1.200.000",
      partner: "Rp 900.000",
      accent: "#B0245B",
    ...
    },
    {
      ...
      category: "peptide",
      categoryLabel: "Research peptide",
      descriptor: "Mitochondrial peptide research product",
      retail: "Rp 1.500.000",
      partner: "Rp 1.200.000",
      accent: "#9B102A",
...
const assetPrefix = pageType.endsWith("retatrutide")
  ? "../../assets/"
  : pageType.startsWith("partner")
    ? "../assets/"
    : "assets/";
const productByKey = Object.fromEntries(
  EQUABOLIX_CONFIG.products.map((product) => [product.key, product]),
);
...
const applyMessageTokens = (message) =>
  message
...
const buildWhatsAppHref = (baseMessage, sourcePage = pageType) => {
  const context = [savedAttribution, `source_page=${sourcePage}`]
    .filter(Boolean)
    .join(" | ");
...
const productLabel = (product) =>
  [product.name, product.strength].filter(Boolean).join(" ");
...
const renderRetailCard = (product, featured = false) => {
  ...
  const strength = product.strength
    ? `<p class="commerce-card__strength">${product.strength}</p>`
    : "";
...
if (featuredRoot)
  featuredRoot.innerHTML = EQUABOLIX_CONFIG.products
    .filter((product) => product.featured)
    .sort((a, b) => a.featured - b.featured)
    .map((product) => renderRetailCard(product, true))
    .join("");

const retailCatalogRoot = document.querySelector("[data-retail-catalog]");
if (retailCatalogRoot)
  retailCatalogRoot.innerHTML = EQUABOLIX_CONFIG.products
    .map((product) => renderRetailCard(product))
    .join("");

const partnerCatalogRoot = document.querySelector("[data-partner-catalog]");
if (partnerCatalogRoot)
  partnerCatalogRoot.innerHTML = EQUABOLIX_CONFIG.products
...
const initWhatsAppLinks = (root = document) => {
  root.querySelectorAll(".js-wa:not([data-wa-ready])").forEach((link) => {
    const intent =
      link.dataset.intent ||
      (pageType.startsWith("partner") ? "partner" : "retail");
    const sourcePage = pageType.startsWith("partner-ads")
      ? pageType
      : link.dataset.sourcePage || pageType;
    const baseMessage =
      link.dataset.message ||
      (intent === "partner"
        ? "Halo Equabolix, saya ingin informasi mengenai Equabolix Partner."
        : "Halo Equabolix, saya ingin bertanya mengenai produk Equabolix.");
    ...
    link.addEventListener("click", () => {
      const payload = {
        intent,
        product: link.dataset.product || "Equabolix catalog",
...
document.querySelectorAll(".catalog-filter").forEach((button) =>
  button.addEventListener("click", () => {
    ...
    document
      .querySelectorAll(`.catalog-filter[data-catalog-scope="${scope}"]`)
      .forEach((item) => {
    ...
    document
      .querySelectorAll(`.commerce-card[data-catalog-scope="${scope}"]`)
      .forEach((card) => {
...
document.querySelectorAll(".js-track").forEach((link) =>
  link.addEventListener("click", () =>
    pushAnalytics(link.dataset.event || "site_link_click", {
      location: link.dataset.location || "unspecified",
...
if (partnerForm)
  partnerForm.addEventListener("submit", (event) => {
    ...
    const message = [
      ...
      "Mohon kirim current availability dan partner pricing.",
    ].join("\n");
    pushAnalytics("partner_qualification_submit", {
      product,
      source_page: pageType,
    });
    window.open(
      buildWhatsAppHref(message, pageType),
      "_blank",
      "noopener,noreferrer",
    );
  });
...
const revealTargets = document.querySelectorAll(
  ".commerce-card,.brand-proof-card,.partner-product-visual,.partner-product-copy,.partner-value-card,.support-list>div,.partner-steps article,.qualification-copy,.qualification-form,.lp-economics__card,.lp-benefit-grid article,.lp-order-flow article",
);
